...
class TestWindowsLaptop:
    ...
    def test_camo_warning_mentions_stealing_signal(self):
        result = self._run()
        camo_warnings = [w for w in result.warnings if "camo" in w.lower()]
        assert camo_warnings
        assert "stealing mic signal" in camo_warnings[0].lower()
