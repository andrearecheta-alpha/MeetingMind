...
BLUETOOTH_KEYWORDS = [
    "airpods", "bluetooth", "wireless", "hands-free ag", "hands-free",
    "galaxy buds", "jabra", "bose", "sony wh-", "sony wf-", "sennheiser",
    "beats", "jbl", "pixel buds", "buds pro", "buds live",
    "find my", "bt ", "buds", "earbuds", "erza", "jade",
]
...
def _categorise(name: str, in_channels: int) -> tuple[DeviceCategory, int, Optional[str]]:
    ...
    if _contains(n, VIRTUAL_KEYWORDS):
        if "camo" in n:
            return DeviceCategory.VIRTUAL_MIC, 2, (
                f"Camo virtual mic detected ({name!r}) — "
                "Camo may be stealing mic signal. "
                "Recommend closing Camo for reliable audio capture"
            )
    ...
    if _contains(n, BLUETOOTH_KEYWORDS):
        return DeviceCategory.REAL_MIC, 12, (
            f"Bluetooth mic detected ({name!r}) — BT mics may have "
            "weaker signal. Consider lowering silence threshold if "
            "transcription drops chunks."
        )
...
def detect_devices() -> DetectionResult:
    ...
    for dev in virtual_mics:
        is_camo = "camo" in dev.name.lower()
        base_msg = (
            "Camo may be stealing mic signal — recommend closing Camo"
            if is_camo
            else "may capture silence or low-quality audio"
        )
        if dev.duplicate_count > 1:
            dev.warning = f"{dev.duplicate_count} virtual instances merged — {base_msg}"
        else:
            dev.warning = f"Virtual mic — {base_msg}"
    ...
    for dev in virtual_mics:
        ...
        if dev.duplicate_count > 1:
            ...
            warnings.append(
                f"Virtual mic '{dev.name}' detected — {base_msg}"
            )
    ...
    if recommended_mic and _contains(recommended_mic.name.lower(), BLUETOOTH_KEYWORDS):
        logger.info(
            "Preferred mic: %s (Bluetooth headset priority applied)",
            recommended_mic.name,
        )
