...
# Maximum documents per ChromaDB collection — oldest are pruned on exceed.
_CHROMA_MAX_DOCS: int = 1000
...
class KnowledgeBase:
    ...
    def _init_chromadb(self) -> None:
        try:
            ...
            for name in ("decisions", "action_items", "risks",
                         "commitments", "open_loops", "summaries", "facts",
                         "pm_global_kb"):
    ...
    def _upsert_collection(
        ...
        try:
            ...
            # Auto-prune oldest documents if collection exceeds limit.
            self._prune_collection(name, col)
        except Exception as exc:
            logger.warning("ChromaDB upsert to '%s' failed: %s", name, exc)

    def _prune_collection(self, name: str, col) -> None:
        """Remove oldest documents if collection exceeds _CHROMA_MAX_DOCS."""
        try:
            count = col.count()
            if count <= _CHROMA_MAX_DOCS:
                return
            excess = count - _CHROMA_MAX_DOCS
            # Fetch all IDs + timestamps, sort by timestamp, delete oldest.
            all_data = col.get(include=["metadatas"])
            ids = all_data["ids"]
            metas = all_data["metadatas"]
            # Sort by timestamp (ISO string) — oldest first; missing = sort first.
            paired = sorted(
                zip(ids, metas),
            ...
            if to_delete:
                ...
                logger.info(
                    "Pruned %d oldest docs from '%s' (was %d, now %d).",
                    len(to_delete), name, count, count - len(to_delete),
                )
        except Exception as exc:
            logger.warning("ChromaDB prune of '%s' failed: %s", name, exc)
    ...
    def ingest_text(
        ...
        """
        Ingest a plain text document into a ChromaDB collection.

        Used for onboarding seed data (user preferences, methodology docs, etc.).

        Args:
            doc_id:         Unique document ID.
            text:           The text content to ingest.
            collection:     Target ChromaDB collection (default: "summaries").
            extra_metadata: Additional metadata to merge into the document metadata.
        ...
        if not text.strip():
            return False
        meta = {"source": "onboarding", "doc_id": doc_id}
        if extra_metadata:
            meta.update(extra_metadata)
        try:
            self._upsert_collection(collection, [doc_id], [text], [meta])
            logger.info("Ingested onboarding text '%s' into '%s'.", doc_id, collection)
            return True
        except Exception as exc:
            logger.warning("Failed to ingest text '%s': %s", doc_id, exc)
            return False

    def pm_global_doc_count(self) -> int:
        """Return the number of documents in the pm_global_kb collection."""
        col = self._collections.get("pm_global_kb")
        if col is None:
            return 0
        try:
            return col.count()
    ...
    def pm_global_files(self) -> list[str]:
        """Return the list of .md filenames in the pm_global directory."""
        pm_dir = Path(__file__).resolve().parents[2] / "data" / "knowledge_base" / "pm_global"
        if not pm_dir.exists():
            return []
        return sorted(f.name for f in pm_dir.glob("*.md"))

    def clear_pm_global_kb(self) -> None:
        """Delete all documents from the pm_global_kb collection."""
        col = self._collections.get("pm_global_kb")
        if col is None:
            return
        try:
            ...
            logger.info("PM global KB cleared (%d docs removed).", len(all_ids))
        except Exception as exc:
            logger.warning("PM global KB clear failed: %s", exc)

    def force_ingest_pm_global_kb(self) -> int:
        """Clear and re-ingest all PM global KB files. Returns section count."""
        self.clear_pm_global_kb()
        # Temporarily bypass the idempotency check in ingest_pm_global_kb
        # by calling the ingestion logic directly.
        col = self._collections.get("pm_global_kb")
        if col is None:
            return 0
        ...
        docs: list[str] = []
        metas: list[dict] = []
        md_files = sorted(pm_dir.glob("*.md"))
        for md_file in md_files:
            text = md_file.read_text(encoding="utf-8")
            sections = [s.strip() for s in text.split("\n## ") if s.strip()]
            for idx, section in enumerate(sections):
                doc_id = f"pm_kb_{md_file.stem}_{idx}"
                ids.append(doc_id)
                docs.append(section[:8000])
                metas.append({"source": "pm_global_kb", "file": md_file.name})

        if not ids:
            return 0

        try:
            self._upsert_collection("pm_global_kb", ids, docs, metas)
            logger.info("PM global KB reseeded: %d sections from %d files.", len(ids), len(md_files))
            return len(ids)
        except Exception as exc:
            logger.warning("PM global KB reseed failed: %s", exc)
            return 0

    def project_doc_count(self) -> int:
        """Return total documents across project-specific collections."""
        total = 0
        for name in ("decisions", "action_items", "risks",
    ...
    def ingest_pm_global_kb(self) -> int:
        """
        Ingest PM methodology .md files from data/knowledge_base/pm_global/.

        Splits each file by ``## `` headers into sections and upserts into
        the ``pm_global_kb`` collection.  Idempotent — skips if already
        populated.

        Returns the number of sections ingested (0 if skipped or failed).
        """
        col = self._collections.get("pm_global_kb")
        if col is None:
            logger.warning("pm_global_kb collection not available — skipping.")
            return 0
        if col.count() > 0:
            logger.info("PM global KB already populated (%d docs) — skipping.", col.count())
            return 0

        pm_dir = Path(__file__).resolve().parents[2] / "data" / "knowledge_base" / "pm_global"
        if not pm_dir.exists():
            logger.info("PM global KB directory not found (%s) — skipping.", pm_dir)
            return 0
        ...
        for md_file in md_files:
            ...
            for idx, section in enumerate(sections):
                doc_id = f"pm_kb_{md_file.stem}_{idx}"
                ids.append(doc_id)
                docs.append(section[:8000])  # cap section length
                metas.append({"source": "pm_global_kb", "file": md_file.name})
        ...
        try:
            self._upsert_collection("pm_global_kb", ids, docs, metas)
            logger.info("PM global KB loaded: %d sections from %d files.", len(ids), len(md_files))
            return len(ids)
        except Exception as exc:
