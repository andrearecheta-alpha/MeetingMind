...
_QUERY_COLLECTIONS = ("decisions", "commitments", "risks", "action_items", "facts", "pm_global_kb")
    ...
    def _format_historical_context(self, items: list[dict]) -> str:
        ...
        for item in items:
            ...
            if col == "decisions":
                ...
                header = f"[Seeded Fact — {label}]"
            elif col == "pm_global_kb":
                source = meta.get("file", "pm_kb")
                header = f"[PM Methodology — {source}]"
            else:
